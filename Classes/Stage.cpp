...
void Stage::moveBlock(Blocks* mirrorBlock, Vec2 mirrorPosition, bool playerRightFlag)
{
	...
	log("%d", (int) playerRightFlag);

	if (playerRightFlag == true)
	{
				...
				if (magicUse == true && moveFlag == false)
				{
					...
					for (Blocks* mirrorBlock : _mirrorAbleBlocks)
					{
						...
						if (_player->magicFlag == true)
						{
							...
							{
								this->moveBlock(mirrorBlock, mirrorPosition, _player->rightFlag);
							}
						}
